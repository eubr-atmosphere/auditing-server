...
cp general.conf auditing-server/database-setup
...
container_id=`sudo docker run --name auditing-server -p 8085:8085 -idt $image`
...
sudo ./generate-ssh-key-pair server-key 2048
